...
object SwapChainLifecycle:
  ...
  def createDescriptorSetLayout()(using stack: MemoryStack) =
...
def loop() =
  ...
  def drawFrame(): Unit = Using.resource(stackPush()) { stack =>
    ...
    def updateUniformBuffer() =
      ...
      ubo.view.get(mat4Size, data)
      ubo.proj.get(mat4Size * 2, data)

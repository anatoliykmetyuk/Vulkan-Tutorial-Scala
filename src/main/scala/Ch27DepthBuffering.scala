...
val vertices = List(
  Vertex(Vector3f(-0.5f, -0.5f, 0.0f), Vector3f(1.0f, 0.0f, 0.0f), Vector2f(1.0f, 0.0f)),
  Vertex(Vector3f(0.5f, -0.5f, 0.0f), Vector3f(0.0f, 1.0f, 0.0f), Vector2f(0.0f, 0.0f)),
  Vertex(Vector3f(0.5f, 0.5f, 0.0f), Vector3f(0.0f, 0.0f, 1.0f), Vector2f(0.0f, 1.0f)),
  Vertex(Vector3f(-0.5f, 0.5f, 0.0f), Vector3f(1.0f, 1.0f, 1.0f), Vector2f(1.0f, 1.0f)),
  ...
  Vertex(Vector3f(-0.5f, 0.5f, -0.5f), Vector3f(1.0f, 1.0f, 1.0f), Vector2f(1.0f, 1.0f)),
)
...
def createImageView(image: Long, format: Int, aspectMask: Int = VK_IMAGE_ASPECT_COLOR_BIT)(using stack: MemoryStack) =  // TODO see if the swapchain image creation can also be abstracted with this
...
def transitionImageLayout(image: Long, format: Int, oldLayout: Int, newLayout: Int)(using stack: MemoryStack): Unit =
...
object SwapChainLifecycle:
  ...
  def createRenderPass()(using stack: MemoryStack) =
    ...
    val attachmentRefs = VkAttachmentReference.callocStack(2, stack)
...
object Allocatable:  // TODO can be simplified via macros
  ...
  given fmtpBuf: Allocatable[VkFormatProperties] = make((_, stack) => VkFormatProperties.callocStack(stack))

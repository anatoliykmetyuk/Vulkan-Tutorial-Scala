...
def initVulkan() = Using.resource(stackPush()) { stack =>
  ...
  def createDescriptorSetLayout() =
    ...
    descriptorSetLayout = create(vkCreateDescriptorSetLayout(device, layoutInfo, null, _: LongBuffer))
  ...
  def createDescriptorPool() =
    ...
    descriptorPool = create(vkCreateDescriptorPool(device, poolInfo, null, _: LongBuffer))
  ...
  createDescriptorSets() // TODO move this to the other description ops. In general, order this init code a bit
  createSyncObjects()
}
...
object SwapChainLifecycle:
  ...
  def createGraphicsPipeline()(using stack: MemoryStack) =
    ...
    val vertShader = compileShader("22_shader_ubo.vert", shaderc_vertex_shader)
    val fragShader = compileShader("22_shader_ubo.frag", shaderc_fragment_shader)

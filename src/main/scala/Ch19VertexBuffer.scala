...
def initVulkan() = Using.resource(stackPush()) { stack =>
  ...
  def createVertexBuffer() =
    ...
    vertexBuffer = create(vkCreateBuffer(device, bufferInfo, null, _: LongBuffer))

    val memRequirements = queryStruct(vkGetBufferMemoryRequirements(device, vertexBuffer, _))
    ...
    vertexBufferMemory = create(vkAllocateMemory(device, allocInfo, null, _: LongBuffer))
    vkBindBufferMemory(device, vertexBuffer, vertexBufferMemory, 0)

    val data = queryStruct(vkMapMemory(device, vertexBufferMemory, 0, bufferInfo.size, 0, _: PointerBuffer))
      .getByteBuffer(0, bufferInfo.size.toInt)

...
var imageAvailableSemaphore: Long = -1
var renderFinishedSemaphore: Long = -1
var inFlightFence: Long = -1
...
def initVulkan() = Using.resource(stackPush()) { stack =>
  ...
  def createCommandBuffers() =
    ...
    commandBuffers = createMany(vkAllocateCommandBuffers(device, allocInfo, _: PointerBuffer),
      commandBuffersCount).map(VkCommandBuffer(_, device))
  ...
  def createSyncObjects() =
    ...
    imageAvailableSemaphore = create(vkCreateSemaphore(device, semaphoreInfo, null, _: LongBuffer))
    renderFinishedSemaphore = create(vkCreateSemaphore(device, semaphoreInfo, null, _: LongBuffer))
    inFlightFence = create(vkCreateFence(device, fenceInfo, null, _: LongBuffer))
